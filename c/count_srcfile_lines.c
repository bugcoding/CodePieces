...
//模拟bool
//typedef enum BOOL{FALSE, TRUE} _BOOL;
...
void list_dir_files(const char *file_name, const char *ext)
{
    ...
    {
        fprintf(stderr, "%s--->", "Get File Error!");
        return ;
    }
    ...
    do
    {
        ...
        {
            char file_real_path[256] = "\0";
            strcat(file_real_path, file_name);
            strcat(file_real_path, "\\");
            strcat(file_real_path, file_data.cFileName);
            list_dir_files(file_real_path, ext);
        }
        else
        {
            //获取文件后缀名
            ...
            if (!strcmp(extension, ext))
            {
                ...
                strcat(file_real_path, file_data.cFileName);
                printf("%s\n", file_path);
                total_lines += get_and_count_lines(file_real_path);
            }
        }
...
int get_and_count_lines(const char *file_name)
{
    ...
    if ((file_in = fopen(file_name, "r")))
    {
        while (!feof(file_in))//读取到文件流的结束
        {
            ...
            {
                //如果读取的这一行不是空行,算入计数
                if (!is_blank_line(buffer) && (!check_is_single_line_comment(buffer, strlen(buffer))
                                               || !check_is_multiple_line_comment(buffer, strlen(buffer))))
                {
                    line_count++;
                }
            }
